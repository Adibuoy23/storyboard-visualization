...
plotting_df['eb_cont_dist_norm_new'] = plotting_df['eb_cont_dist_new']/plotting_df['eb_cont_dist_new'].max()
plotting_df['sb_cont_dist_norm_new'] = plotting_df['sb_cont_dist_new']/plotting_df['sb_cont_dist_new'].max()
...
def plot_distributions(plotting_df, clip_number=0, type='Map max to 1'):
    if type == 'Map max to 1':
        ...
        x3 = 'sb_cont_dist_norm_new'
        x4 = 'eb_cont_dist_norm_new'
    ...
    dist_fig.add_trace(go.Scatter(x=np.arange(len(clip_data)), y=clip_data[x2], name='Event boundary (lab)', mode='lines', fill='tozeroy', line=dict(color='rgba(231,41,138, 0.35)', width=2)))
    dist_fig.add_trace(go.Scatter(x=np.arange(len(clip_data)), y=clip_data[x4], name='Event boundary (online)', mode='lines', fill='tozeroy', line=dict(color='gray', width=2)))
